...
```{r}
#| label: Corals_Species_by_Order_per_location_dark_blue

# prepare data: unique species per location × host_order (within Hexacorallia & Octocorallia)
prop_df <- Corals_clean %>%
  filter(!is.na(location),
         !is.na(host_order),
         !is.na(host_species),
...
# expanded custom blue palette (6 shades)
blue_palette <- c("#ebf2ff", "#c2d9ff", "#a4c2f4", "#6d9eeb", "#3c78d8", "#1c4587")

# 100% stacked bar chart with custom fills
ggplot(prop_df, aes(x = location, y = prop, fill = host_order)) +
  geom_col(position = "fill", width = 0.6) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = blue_palette, name = "Host Order") +
  labs(
    title = "Proportion of Coral Species by Order",
    x = "Location",
    y = "Proportion of Unique Species"
  ) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, size = 16, face = "bold", color = "white"), # centered
...
```{r}
#| label: Species_Richness_by_Symbiont_Potential_dark_blue
...
# Consistent blue palette (6 shades in case there are multiple categories)
blue_palette <- c("#ebf2ff", "#c2d9ff", "#a4c2f4", "#6d9eeb", "#3c78d8", "#1c4587")

# Bar plot with dark background + blue scheme
ggplot(symbiont_richness, aes(x = symbiont.potential, 
                              y = Richness, 
                              fill = symbiont.potential)) +
  geom_col(width = 0.7) +
  ...
  theme(
    ...
    legend.position = "none" # keep legend off if not needed
  )
...
```{r}
#| label: entropy_location_all_species
...
```{r}
#| label: coral_entropy_per_location
...
# Pooled Shannon entropy per location (only Hexacorallia & Octocorallia)
entropy_location <- Corals_clean %>%
  filter(!is.na(location),
         !is.na(host_species),
         host_class %in% c("Hexacorallia", "Octocorallia")) %>%   # <-- filter here
  count(location, host_species, name = "abundance") %>%
  group_by(location) %>%
  summarise(
    Richness = n_distinct(host_species),
...
# Custom harmonious blue palette
blue_palette <- c("#ebf2ff", "#c2d9ff", "#a4c2f4", "#6d9eeb", "#3c78d8", "#1c4587")

# Bar plot with dark theme
ggplot(entropy_location, aes(x = location, y = Shannon, fill = location)) +
  geom_col(width = 0.7) +
  scale_fill_manual(values = blue_palette) +
  labs(title = "Coral Species Entropy by Location",
       x = "Location",
       y = "Diversity Index") +
  theme_minimal(base_size = 14) +
  theme(
...
metabolite_summary <- metabolite_data %>%
  ...
  group_by(metabolite) %>%
  summarise(
    ubiquity = mean(value > 0) * 100,       # % samples where present
