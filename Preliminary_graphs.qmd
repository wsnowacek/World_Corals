...
```{r}
#| label: Metabolite_ubiquity_vs_average_abundance_corals_total
...
# Helper function
summarize_metabolites <- function(df, group_name) {
  df %>%
    select(starts_with("x")) %>%
...
# Summaries
met_summary_all <- summarize_metabolites(Corals_clean, "All hosts")

met_summary_corals <- Corals_clean %>%
  filter(host_class %in% c("Hexacorallia", "Octocorallia")) %>%
  summarize_metabolites("Corals")

met_summary_combined <- bind_rows(met_summary_all, met_summary_corals)

# Plot
ggplot(met_summary_combined, aes(x = ubiquity, y = avg_abundance, color = group)) +
  geom_point(alpha = 0.8, size = 2) +
  theme_minimal(base_size = 14) +
  scale_color_manual(values = c("Corals" = "#1f78b4", "All hosts" = "#e31a1c"), name = NULL) + # remove legend title
  scale_y_continuous(labels = label_number_si()) +
  scale_x_continuous(
    limits = c(0, 100),
    breaks = seq(0, 100, by = 20),  # fewer ticks
    labels = as.character(seq(0, 100, by = 20)) # remove percent sign
  ) +
  labs(
    x = "Ubiquity (% samples present)",
    y = "Average abundance",
