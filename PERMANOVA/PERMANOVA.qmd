...
pcoa_location_result <- cmdscale(bray_curtis, eig = TRUE, k = 2)

# Extract coordinates
pcoa_location <- as.data.frame(pcoa_location_result$points)
colnames(pcoa_location) <- c("PCoA1", "PCoA2")

# Combine coordinates with metadata
pcoa_location <- cbind(pcoa_location, meta_data)

# Percent variance explained by first two axes
pos_eig <- pcoa_location_result$eig[pcoa_location_result$eig > 0]
var_explained <- round(100 * pcoa_location_result$eig[1:2] / sum(pos_eig), 1)

#  Plot with 95% confidence ellipses
ggplot(pcoa_location, aes(x = PCoA1, y = PCoA2, color = location, fill = location)) +
  geom_point(size = 3, alpha = 0.8) +
