...
ubiquity_avg_abundance_values <- Corals_clean %>%
...
plot_data <- merged_metabolite_data %>%
  filter(npc_number_pathway != "Unknown")

ggplot(plot_data,
       aes(x = ubiquity, y = avg_abundance, color = npc_number_pathway)) +
  geom_point(size = 2.5, alpha = 0.6) +
  scale_y_continuous(labels = y_labels_fn) +
  
  # ---- Fixed 7-category color palette ----
  scale_color_manual(
    values = c(
      "Fatty acids"                     = "#E69F00",
      "Polyketides"                     = "#56B4E9",
  ...
  theme(
    ...
    plot.title        = element_text(hjust = 0.5, face = "bold", color = "white"),
    plot.subtitle     = element_text(hjust = 0.5, color = "white", size = 12, margin = margin(b = 6)),
    ...
    # make plot area bigger by reducing outer margin
    plot.margin       = margin(t = 10, r = 10, b = 10, l = 10)
  )
```

```{r}
#| label: ubiquity-graph-npc-pathway-host-only

# Filter merged_metabolite_data to only Host rows
plot_data_host <- merged_metabolite_data %>%
  filter(refined_origin == "Host") %>%
  filter(npc_number_pathway != "Unknown")   # Unknown removed

ggplot(plot_data_host,
       aes(x = ubiquity,
           y = avg_abundance,
           color = npc_number_pathway)) +
  geom_point(size = 2.5, alpha = 0.6) +
  scale_y_continuous(labels = y_labels_fn) +
  
  # ---- Same 7-color pathway palette ----
  scale_color_manual(
    values = c(
      "Fatty acids"                     = "#E69F00",
      "Polyketides"                     = "#56B4E9",
  ...
  labs(
    x = "Ubiquity (% samples present)",
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Host only)",
    subtitle = "by pathways of NPClassifiers"
  ) +
  ...
  theme(
    ...
    # Title + subtitle styling (critical for visibility)
    plot.title        = element_text(hjust = 0.5, face = "bold", color = "white"),
    plot.subtitle     = element_text(hjust = 0.5, color = "white", size = 12, margin = margin(b = 6)),
...
```{r}
#| label: ubiquity-graph-npc-pathway-symbiont-only

plot_data_symbiont <- merged_metabolite_data %>%
  filter(refined_origin == "Symbiont") %>%
  filter(npc_number_pathway != "Unknown")   # removes Unknown 

ggplot(plot_data_symbiont,
       aes(x = ubiquity,
           y = avg_abundance,
           color = npc_number_pathway)) +
  geom_point(size = 2.5, alpha = 0.6) +
  ...
  # ---- Same 7-color palette as other plots ----
  scale_color_manual(
    values = c(
      "Fatty acids"                     = "#E69F00",
      "Polyketides"                     = "#56B4E9",
  ...
  labs(
    x = "Ubiquity (% samples present)",
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Symbiont only)",
    subtitle = "by pathways of NPClassifiers"
  ) +
...
```{r}
#| label: ubiquity-graph-npc-pathway-both

# Filter merged_metabolite_data to only Both rows
plot_data_both <- merged_metabolite_data %>%
...
ggplot(plot_data_both,
  ...
  # ---- Same 7-color pathway palette used everywhere ----
  scale_color_manual(
    values = c(
      "Fatty acids"                     = "#E69F00",
      "Polyketides"                     = "#56B4E9",
  ...
  labs(
    x = "Ubiquity (% samples present)",
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Both)",
    subtitle = "by pathways of NPClassifiers"
  ) +
  ...
  theme(
    ...
    # Title + subtitle must be styled for visibility
    plot.title        = element_text(hjust = 0.5, face = "bold", color = "white"),
    plot.subtitle     = element_text(hjust = 0.5, color = "white", size = 12, margin = margin(b = 6)),
...
## Scleractinia Ubiquity graphs by metabolite compound class

About 60% of the samples are Scleractinia; ubiquity on this plot only reflects % out of Scleractinia samples.

```{r}
#| label:  ubiquity-graph-npc-pathway-scleractinia-only

# compute ubiquity among Scleractinia samples only
ubiquity_scler_df <- Corals_clean %>%
  # select metabolite columns and sample metadata needed
  select(host_order, starts_with("x")) %>%
  pivot_longer(
    cols = starts_with("x"),
    names_to = "metabolite",
    values_to = "value"
  ...
  group_by(metabolite) %>%
  summarise(
    ubiquity_scler = mean(value > 0) * 100,
    .groups = "drop"
  )

# get only shared metabolites 
shared_metabolites <- merged_metabolite_data %>%
  semi_join(met_summary_coral_only, by = "metabolite")

# join the scleractinia-only ubiquity percent
plot_data_shared_scler <- shared_metabolites %>%
  left_join(ubiquity_scler_df, by = "metabolite") %>%
  # if any metabolites are missing (no scleractinia samples with that metabolite),
  # set ubiquity to 0 
  mutate(ubiquity_scler = ifelse(is.na(ubiquity_scler), 0, ubiquity_scler)) %>%
  # drop Unknown pathway 
  filter(npc_number_pathway != "Unknown")

# Plot using ubiquity_scler on the x-axis
ggplot(plot_data_shared_scler,
       aes(x = ubiquity_scler,
           y = avg_abundance,
           color = npc_number_pathway)) +
  geom_point(size = 2.5, alpha = 0.6) +
  scale_y_continuous(labels = y_labels_fn) +
  scale_x_continuous(
    name = "Ubiquity (% Scleractinia samples present)",
    limits = c(0, 100),
    breaks = seq(0, 100, by = 20),
    labels = as.character(seq(0, 100, by = 20))
  ) +
  
  # ---- Same 7-color pathway palette & legend spec ----
  scale_color_manual(
    values = c(
      "Fatty acids"                     = "#E69F00",
      "Polyketides"                     = "#56B4E9",
  ...
  labs(
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Scleractinia)",
    subtitle = "by pathways of NPClassifiers"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    ...
    # Title + subtitle styling (visible on dark bg)
    plot.title        = element_text(hjust = 0.5, face = "bold", color = "white"),
    plot.subtitle     = element_text(hjust = 0.5, color = "white", size = 12, margin = margin(b = 6)),
...
```{r}
#| label:  ubiquity-graph-npc-pathway-scleractinia-hostorigin-only
...
# get only shared metabolites AND restrict to refined_origin == Host
shared_metabolites <- merged_metabolite_data %>%
  filter(refined_origin == "Host") %>%        # <-- NEW minimal line
  semi_join(met_summary_coral_only, by = "metabolite")
...
ggplot(plot_data_shared_scler,
  ...
  labs(
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Host)",
    subtitle = "by pathways of NPClassifiers"
  ) +
...
```{r}
#| label:  ubiquity-graph-npc-pathway-scleractinia-symbiontorigin-only
...
# get only shared metabolites AND restrict to refined_origin == Symbiont
shared_metabolites <- merged_metabolite_data %>%
  filter(refined_origin == "Symbiont") %>%        # <-- NEW minimal line
  semi_join(met_summary_coral_only, by = "metabolite")
...
ggplot(plot_data_shared_scler,
  ...
  labs(
    y = "Average abundance",
    title = "Metabolite ubiquity vs average abundance (Symbiont)",
    subtitle = "by pathways of NPClassifiers"
  ) +
...
```{r}
#| label:  ubiquity-graph-npc-pathway-scleractinia-both
...
# get only shared metabolites AND restrict to refined_origin == Both
shared_metabolites <- merged_metabolite_data %>%
  filter(refined_origin == "Both") %>%        # <-- NEW minimal line
  semi_join(met_summary_coral_only, by = "metabolite")
