...
ggplot(plot_data,
  ...
  theme(
    ...
    legend.position   = "bottom",
    legend.title      = element_blank(),
    legend.text       = element_text(color = "white", size = 8),   # smaller text
    legend.key        = element_rect(fill = "#000042", color = NA),
    legend.spacing.y  = unit(0.15, "cm"),
    legend.spacing.x  = unit(0.2, "cm"),
    legend.margin     = margin(t = 0, b = 0),                      # reduce extra padding
    legend.box.margin = margin(t = -5),                            # pulls legend closer
...
# get only shared metabolites (as you had)
...
plot_data_shared_scler <- shared_metabolites %>%
  ...
  # set ubiquity to 0 (or keep NA if you prefer)
  mutate(ubiquity_scler = ifelse(is.na(ubiquity_scler), 0, ubiquity_scler)) %>%
  # drop Unknown pathway if desired
  filter(npc_number_pathway != "Unknown")
