...
```{r}
# Calculation to use 2.25 standard deviations below the mean for richness
...
richness_qc <- richness_qc %>%
  mutate(
    ...
    # Define threshold: standard deviations below the mean
...
richness_qc_clean <- richness_qc %>%
  filter(is_outlier != "Low Richness")

# Verify the removal
# This should show 0 "Low Richness" entries
table(richness_qc_clean$is_outlier)

# Check how many samples were removed
print(paste("Samples removed:", nrow(richness_qc) - nrow(richness_qc_clean)))
```
...
# 1) Long-format presence/values for all samples
met_presence_long <- richness_qc_clean %>%
  pivot_longer(
    cols = starts_with("x"),
    names_to = "metabolite",
    values_to = "value"
...
taxonomic_counts <- richness_qc_clean %>%
  summarise(across(
...
```{r}
avg_metabolite_values_family <- richness_qc_clean |>
